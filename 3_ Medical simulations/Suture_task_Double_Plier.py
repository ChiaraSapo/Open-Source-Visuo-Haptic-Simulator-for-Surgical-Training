...
def createScene(root):
    ...
    drawDevice="0", positionBase="20 6 10",  orientationBase="0.707 0 0 0.707", forceFeedBack="@SutureNeedle/LCPFFNeedle")

    root.addObject('GeomagicDriver', name="GeomagicDeviceLeft", deviceName="Left Device", scale="1", drawDeviceFrame="1", 
    drawDevice="0", positionBase="0 6 10",  orientationBase="0.707 0 0 0.707", forceFeedBack="@SutureNeedleLeft/LCPFFNeedle")
    ...
    suture_models.Plier(parentNode=root, name='SutureNeedleLeft', monitor=True, file1=fileNamePosLeft, file2=fileNameVelLeft, file3=fileNameForceLeft, position="@GeomagicDeviceLeft.positionDevice", external_rest_shape='@../OmniLeft/DOFs', rx=0, ry=180, rz=0) # To fall on sphere: dx=12, dy=3, dz=6
...
class SutureTrainingContactControllerDoubleHaptic(Sofa.Core.Controller):
    ...
    def onAnimateBeginEvent(self, event):
        ...
        if coll_indexes!=[] and self.finished==True:
            ...
            if (coll_index_skin in suture_models.Skin.sphere1Box.findData('triangleIndices').value) and (self.root.GeomagicDeviceRight.findData('positionDevice').value[2]<=1):
                print("Box 1")    
            ...
            elif (coll_index_skin in suture_models.Skin.sphere2Box.findData('triangleIndices').value) and (self.root.GeomagicDeviceRight.findData('positionDevice').value[2]<=1):
                print("Box 2")
                # Change sphere color
        ...
        if self.root.GeomagicDeviceLeft.findData('button2').value!=0:
