...
carving=False, side=0, task=None):
    ...
    #################### BEHAVIOUR ##########################
    # Solvers
    name.addObject('EulerImplicitSolver', name="odesolver", rayleighStiffness="0.1", rayleighMass="0.1") #added  2 params
    ...
    name.addObject('MechanicalObject', name='SkinMechObj', src='@volumeLoader', template='Vec3d', translation=translation) #restScale="1"
    ...
    name.addObject('DiagonalMass', name="SkinMass", massDensity="0.5", template="Vec3d,double") #density: higher: pudding.

    # Forces
    name.addObject('TetrahedralCorotationalFEMForceField', template='Vec3d', name='FEM', method='large', youngModulus=360, poissonRatio=0.3)
    name.addObject('FixedPlaneConstraint', template="Vec3d", name="default4", direction="0 0 1", dmin="-0.1", dmax="0.1")
    #name.addObject('FixedConstraint', template="Vec3d", name="default5", indices="0" )
    name.addObject('TetrahedronCollisionModel')
    ...
    SkinColl.addObject('TriangularFEMForceField', template="Vec3d", name="FEM", method="large", poissonRatio="0.3", youngModulus="60")
    SkinColl.addObject('TriangularBendingSprings', template="Vec3d", name="FEM-Bend", stiffness="300", damping="1")
    SkinColl.addObject('TrianglePressureForceField', template="Vec3d", name="default7", pressure="0.4 0 0", normal="0 0 1", dmin="0.9", dmax="1.1") 

    #SkinColl.addObject('TriangleCollisionModel', name="TriangleCollisionSkin") # but i need this....
...
def physicalSphere(parentNode=None, name=None, translation=[0.0, 0.0, 0.0], scale3d=[0.0, 0.0, 0.0], color=[0.0, 0.0, 0.0]):
    ...
    name.addObject('MechanicalObject', template="Rigid3d", scale="1.0" ,translation=[5, 10, 5])
