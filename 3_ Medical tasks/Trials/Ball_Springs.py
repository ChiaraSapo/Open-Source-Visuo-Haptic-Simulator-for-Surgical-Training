...
def createScene(root):
    ...
    physicalSphere1(parentNode=root, name="spherebassa", translation=[0,0,-3])
    physicalSphere2(parentNode=root, name="spherealta", translation=[0,0,4])

    physicalSphere3(parentNode=root, name="spherbassasx", translation=[-5,-5,-3])
    physicalSphere4(parentNode=root, name="spherealtadx", translation=[-5,-5,4])
    ...
    spring_force_field_skins = root.addObject("StiffSpringForceField", name="SkinsFF", object1 = physicalSphere1.MO1,  object2=physicalSphere2.MO2)
    springs = [Sofa.SofaDeformable.LinearSpring(index1=i, index2=j, springStiffness=50, dampingFactor=1, restLength=1) for i, j in zip(range(150),range(150))] 
    spring_force_field_skins.addSprings(springs)

    # spring_force_field_skins2 = root.addObject("StiffSpringForceField", name="SkinsFF", object1 = physicalSphere3.MO1,  object2=physicalSphere4.MO2)
    # springs2 = [Sofa.SofaDeformable.LinearSpring(index1=i, index2=j, springStiffness=1000, dampingFactor=1, restLength=1) for i, j in zip(range(150),range(150))] 
    # spring_force_field_skins2.addSprings(springs2)
...
def physicalSphere3(parentNode=None, name=None, translation=[0.0, 0.0, 0.0]):
    ...
    name.addObject('UncoupledConstraintCorrection')
    #Force = name.addObject('ConstantForceField', name="CFF", totalForce=[0, 0, 1, 0, 0, 0] )
    name.addObject("Monitor", name="trial1", indices="0 1 2 3", listening="1", showPositions="1", PositionsColor="1 1 0 1", TrajectoriesPrecision="0.1", TrajectoriesColor="1 1 0 1", ExportPositions="true")

    physicalSphere3.MO1=name.InstrumentMechObject.getLinkPath()
...
def physicalSphere4(parentNode=None, name=None, translation=[0.0, 0.0, 0.0]):
    ...
    physicalSphere4.MO2=name.InstrumentMechObject.getLinkPath()
    boxROI=name.addObject('BoxROI', name='boxROI', box=[-8,-8,2, -2,-2,7], drawBoxes='true', computeTriangles='true')
    name.addObject('RestShapeSpringsForceField', name='rest', points='@boxROI.indices', stiffness='1e12', angularStiffness='1e12')
    #################### COLLISION ##########################
