...
for i in range(1,3):
    ...
    percentage=temp*100/(posx.size)
    plt.text(0,7,f"{int(percentage)} %", fontsize=14)
...
for i in range(7,12):
    ...
    triangle1 = Rectangle( (0,3), data_pos['Time'].size-1, 2, fc=(0.5,0.3,1,0.5), ec=(0,0,0,0), lw=2)
    ax.add_artist(triangle1)
    temp=data_pos['Time']
    plt.xlabel('Time[dt]')
    plt.ylabel('Height[cm]')
    ...
    percentage=temp*100/(posx.size)
    plt.text(1,9,f"{int(percentage)} %", fontsize=14)
#plt.show()
...
for i in range(12,20):
    ...
    plt.plot(data_pos['Time'], data_pos['z1'],  color="palegreen", linewidth="2",zorder=2)
    #plt.plot(data_pos['Time'], height, color="pink", linewidth="3")
    temp=data_pos['Time']
    print(temp)
    tr=Rectangle((-0.5,0),50, 1.5,color="pink",zorder=1)
    ax.add_artist(tr)
    plt.xlabel('Time[dt]')
    plt.ylabel('Height[cm]')
    plt.xlim([0,temp[temp.size-1]])
